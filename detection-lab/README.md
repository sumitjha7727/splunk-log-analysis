...
any of the rules in `07-detection-as-code/`. This project exists
...
This lab carries forward `splunk-app/` and all of Project 7's Sigma
rules as its literal starting point, not just a thematic one —
...
- **`splunk-app/`** — carried forward from Project 7's `splunk-app/`;
  ...
  sourcetypes.
- **`detections/`** — all eleven Sigma rule files ported from
  `07-detection-as-code/` as the starting set, covering six
  detections. ("Six Sigma rules" per this project's original scope
  note refers to the six detections *rows* in that project's table —
  the SSH detection alone spans four correlated files (a base failure
  event, an `event_count` threshold correlation over it, a base
  success event, and the `temporal_ordered` correlation tying them
  together), and the DHCP and DNS-tunneling detections each split into
  a base event plus an `event_count`/`value_count` correlation too —
  see `07-detection-as-code/README.md`'s per-detection "Sigma
  modeling" notes for why. Porting fewer files than the complete set
  would break those correlations' dependencies on their component
  rules.)
...
against a JSON fixture. That's accurate for rules with a plain
`detection:` block (the two scanner-signature rules, the
injection-probe rule, and the four broad base-event rules), but it
cannot evaluate any of the four `correlation:`-type rules at all (the
SSH threshold, DHCP anomaly, and DNS-tunneling `event_count`/
`value_count` correlations, plus the `temporal_ordered`
brute-force-then-success rule) — those are explicitly called out and
skipped, not silently mis-evaluated, in the test file itself. A green
CI run confirms the ported rules are syntactically valid and that
simple selections match their fixtures; it is not a substitute for
actually measuring detection accuracy against a real
...
2. **Port one detection** — start with `nmap-scan-signature.yml` (the
   simplest single-event selection in the set — no correlation, no
   base rule to wire up first) and confirm it fires against a
   manually-triggered event in the live lab before trying to scale to
   the rest of the set.
...
6. **Scale to roughly 15 rules across DNS/process/auth** — once the
   full pipeline above is proven end-to-end on the starting set,
   expand `detections/` with new rules spanning process-creation
